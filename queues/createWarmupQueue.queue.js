...
const { createWarmupQueueJob } = require('../jobs/createWarmupQueue.job');

const REDIS_PORT = process.env.REDIS_PORT || 6379
const REDIS_HOST_NAME = process.env.REDIS_HOST_NAME || "127.0.0.1"
const REDIS_PASSWORD = process.env.REDIS_PASSWORD || ""

const queueName = "createWarmupEmailQueue";
...
queue.process(createWarmupQueueJob);
...
const addJobToQueue = async (arguments) => {
    try {
        ...
        // logger.log('info', `${queueName}: Job ${job.id} for provider ${arguments?.senderProviderData._id} with receiver ${arguments?.receiverProviderData._id} of user ${arguments?.userId}} is added to the queue.`);
